...
export default function CardComponent({ 
  ...
  precoSemDesconto 
   ...
   // Verifica se a tag contém um valor numérico seguido de '%' (desconto)
   const isDiscount = tag.includes('%');
   // Verifica se a tag é "NEW"
   const isNew = tag.toUpperCase() === 'NEW';
 
   // Define a cor da bolinha: vermelha se for desconto, verde se for "NEW"
   const bgColor = isDiscount ? "bg-[#E97171]" : isNew ? "bg-[#2EC1AC]" : "bg-gray-300"; // Cor padrão (cinza) caso não seja nem desconto nem "NEW"
    ...
    return (
      <div className="w-[295px] h-[446px] flex gap-2 flex-col bg-[#F4F5F7] font-poppins p-2 relative">
        <div className="relative">
          ...
          <div
          className={`absolute top-2 right-2 w-14 h-14 flex items-center justify-center rounded-full text-white text-[16px] font-semibold ${bgColor}`} // Aplica a cor dinâmica aqui
        >
          {tag}
        </div>
        </div>
